...
// ─────────────────────────────────────────────────────────────────────────────
// components/dashboard/MostExpensiveOwned.tsx
//
// Dashboard card that shows the single most expensive Pokémon card the
// authenticated user currently owns, sourced from
    ...
    <div className="bg-elevated border border-subtle rounded-xl px-6 py-8 flex flex-col items-center text-center gap-3">
      ...
      <div>
        ...
        <p className="text-sm text-secondary max-w-xs mx-auto leading-relaxed">
          Add cards to your collection to see your most valuable one here.
        </p>
      </div>
    </div>
  )
...
// ── Main component ─────────────────────────────────────────────────────────────
export default function MostExpensiveOwned() {
  const { user, profile } = useAuthStore()
  const currency: string = (profile as Record<string, unknown> | null)?.preferred_currency as string ?? 'USD'

  // undefined = loading; null = loaded but no card; OwnedCard = data
  const [card, setCard] = useState<OwnedCard | null | undefined>(undefined)

  useEffect(() => {
    if (!user) return
    let cancelled = false

    fetch('/api/dashboard/most-expensive-owned')
      .then(async (res) => {
        if (!res.ok) throw new Error(`${res.status}`)
        return res.json() as Promise<{ card: OwnedCard | null }>
      })
      .then((data) => {
