...
import type { SetSymbolGridItem } from './SetSymbolGrid'
...
interface Props {
  set: SetSymbolGridItem | null
  isOpen: boolean
  onClose: () => void
  onUploadSuccess: (setId: string, symbolUrl: string) => void
  onNextSet?: () => void
  hasNextSet?: boolean
}
 ...
 * Reuses the same /api/proxy-image route used for card and set logo images.
 */
async function fetchImageViaProxy(imageUrl: string): Promise<File> {
  ...
  const fileName = urlPath.split('/').pop() || 'set-symbol.png'
  return new File([blob], fileName, { type: contentType })
}

export function SetSymbolUploadModal({ set, isOpen, onClose, onUploadSuccess, onNextSet, hasNextSet }: Props) {
  ...
  const reset = () => {
    ...
    setUploadState({ uploading: false, success: false, error: null, symbolUrl: null })
  }
  ...
  const handleUpload = async () => {
    ...
    setUploadState({ uploading: true, success: false, error: null, symbolUrl: null })
    ...
    try {
      ...
      const response = await fetch('/api/upload-set-symbol', {
        method: 'POST',
        body: formData,
      })
      ...
      setUploadState({ uploading: false, success: true, error: null, symbolUrl: result.symbolUrl })
      onUploadSuccess(set.id, result.symbolUrl)
    } catch (err) {
      setUploadState({
        uploading: false,
        success: false,
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          ...
          >
            <DialogPanel className="w-full max-w-lg bg-gray-950 border border-gray-800 rounded-2xl shadow-2xl p-6 space-y-5">
              ...
              {/* Current symbol preview (if any) */}
              {set?.symbol_url && !uploadState.success && (
                <div className="flex items-center gap-3 p-3 bg-gray-900 rounded-lg border border-gray-800">
                  ...
                  />
                  <p className="text-green-400 text-sm">✅ Set already has a symbol — uploading will replace it</p>
                </div>
              )}
              ...
              {uploadState.success ? (
                <div className="space-y-4">
                  <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg text-center">
                    <p className="text-green-400 font-semibold text-lg">✅ Symbol uploaded!</p>
                    {uploadState.symbolUrl && (
                      <div className="mt-3 flex justify-center">
                        <img
                          src={uploadState.symbolUrl}
                          alt="Uploaded symbol"
                          className="h-16 w-16 object-contain rounded shadow-lg"
                        />
                      </div>
                    )}
                  </div>
