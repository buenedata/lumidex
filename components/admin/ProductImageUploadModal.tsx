...
import type { ProductGridItem } from './ProductImageGrid'
...
interface Props {
  ...
  isOpen: boolean
  onClose: () => void
  onUploadSuccess: (productId: string, imageUrl: string) => void
  onNextProduct?: () => void
...
export function ProductImageUploadModal({
  ...
  const handleLoadFromUrl = async () => {
    ...
    if (!isAcceptedImageUrl(trimmed)) {
      setUploadState((prev) => ({
        ...prev,
        error: 'URL must be an https image from Collectr, TCGCollector, or pkmn.gg',
      }))
      return
    }
  ...
  const handleUpload = async () => {
    ...
    if (response.ok && result.success) {
      ...
      onUploadSuccess(product.id, result.imageUrl)
    } else {
      setUploadState({
        uploading: false,
        success: false,
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          ...
          >
            <DialogPanel className="w-full max-w-lg bg-gray-950 border border-gray-800 rounded-2xl shadow-2xl p-6 space-y-5">
              ...
              {product?.image_url && !uploadState.success && (
                <div className="flex items-center gap-3 p-3 bg-gray-900 rounded-lg border border-gray-800">
                  ...
                  />
                  <p className="text-green-400 text-sm">✅ Product already has an image — uploading will replace it</p>
                </div>
              )}
                    ...
                    {uploadState.imageUrl && (
                      <div className="mt-3 flex justify-center">
                        <img
                          ...
                          className="h-32 w-auto rounded shadow-lg object-contain"
                        />
                      </div>
                    )}
                  </div>
                    ...
                    {fetchingUrl ? (
                        ...
                        {urlPreviewSrc && (
                          <div className="flex justify-center mb-3">
                            <img
                              src={urlPreviewSrc}
                              alt="Preview"
                              className="h-28 w-auto rounded shadow-md object-contain"
                            />
                          </div>
                        )}
                      ...
                      <div className="space-y-1 pointer-events-none">
                        ...
                        <p className="text-white font-medium">
                          Drag image from{' '}
                          <span className="text-yellow-400">Collectr</span>
                        </p>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="https://public.getcollectr.com/…"
                    ...
                    {urlInput.length > 8 && !urlInputValid && (
                      <p className="text-yellow-600 text-xs mt-1">
                        ⚠️ Must be an https URL from Collectr, TCGCollector, or pkmn.gg
                      </p>
                    )}
                  </div>
