...
interface Props {
  onSetSelect: (set: SetSymbolGridItem) => void
...
export function SetSymbolGrid({ onSetSelect, onSetsLoaded, selectedSetId, refreshKey = 0 }: Props) {
  ...
  const withSymbol = sets.filter((s) => !!s.symbol_url).length
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between text-sm">
        <span className="text-gray-400">
          <span className="text-green-400 font-semibold">{withSymbol}</span>
          {' / '}
          ...
          <div className="flex items-center gap-2">
            <div className="h-1.5 w-32 bg-gray-700 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(withSymbol / total) * 100}%` }}
              />
            </div>
            <span className="text-gray-500 text-xs">
              {Math.round((withSymbol / total) * 100)}%
            </span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-1.5 max-h-[60vh] overflow-y-auto pr-1">
        {filtered.map((set) => {
          const hasSymbol  = !!set.symbol_url
          ...
          return (
            <button
              ...
              `}
            >
              {/* Symbol thumbnail or logo fallback or placeholder */}
              <div className="w-10 h-10 rounded flex-shrink-0 overflow-hidden bg-gray-800 border border-gray-700 flex items-center justify-center">
                {hasSymbol ? (
                  <img
                    src={set.symbol_url!}
                    alt={`${set.name} symbol`}
                  ...
                  <img
                    src={set.logo_url}
                    alt={set.name}
                    className="w-full h-full object-contain p-0.5 opacity-40"
                    loading="lazy"
                  />
                ) : (
