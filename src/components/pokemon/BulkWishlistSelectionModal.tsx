...
import { Star, X, Heart, Check, ChevronsUpDown, Package, Plus, List } from 'lucide-react';
...
interface BulkWishlistSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  cardIds: string[];
  cardCount: number;
...
export const BulkWishlistSelectionModal: React.FC<BulkWishlistSelectionModalProps> = ({
  isOpen,
  onClose,
  cardIds,
  cardCount,
  ...
  const handleAddToWishlist = async (wishlistId: string) => {
    ...
    try {
      ...
      // Add cards to the specified wishlist using bulk operation
      const result = await wishlistListsService.addCardsToWishlistList(wishlistId, cardIds, {
      ...
      if (result.success) {
        ...
        if (addedCount > 0) {
          showSuccess(
            'Cards added to wishlist!',
            `${addedCount} cards from ${setName} have been added to your wishlist${skippedCount > 0 ? ` (${skippedCount} were already in your wishlist)` : ''}`
          );
          onClose();
        } else {
          showError('No cards added', result.error || 'All cards are already in your wishlist');
        }
      } else {
        showError('Failed to add cards to wishlist', result.error || 'Please try again later');
      }
    } catch (error) {
      console.error('Error adding cards to wishlist:', error);
  ...
  const handleCreateNewList = async () => {
    ...
    try {
      ...
      // Add all cards to the new wishlist
      let addedCount = 0;
      let skippedCount = 0;
      
      for (const cardId of cardIds) {
        try {
          const result = await wishlistListsService.addCardToWishlistList(createResult.data.id, cardId, {
            priority: selectedPriority.id as 1 | 2 | 3 | 4 | 5,
            maxPriceEur,
            conditionPreference: selectedCondition.id as 'any' | 'mint' | 'near_mint' | 'lightly_played' | 'moderately_played',
            notes: notes || undefined
      ...
      if (addedCount > 0) {
        showSuccess('Wishlist created!', `${addedCount} cards from ${setName} have been added to "${newListName}"`);
        setNewListName('');
        setNewListDescription('');
        setShowCreateNew(false);
        onClose();
      } else {
        showError('Failed to add cards', 'Please try again later');
      }
    } catch (error) {
                ...
                <div className="p-6 border-b border-gray-700/50">
                  <div className="flex items-center space-x-4">
                    <div className="relative">
                      <div className="w-16 h-22 bg-pkmn-surface rounded-lg border border-gray-600 flex items-center justify-center">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-white text-lg">
                        {cardCount} cards from {setName}
                      </h3>
                      <p className="text-sm text-gray-400">Set your preferences for all cards</p>
                    </div>
                  </div>
                </div>
                          ...
                          <div className="p-6 bg-pkmn-surface/50 rounded-xl border border-gray-600 space-y-4">
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="flex space-x-3">
                                ...
                                >
                                  {adding ? 'Creating...' : `Create & Add ${cardCount} Cards`}
                                </button>
                                <button
                      ...
                      <div className="space-y-6 pt-6 border-t border-gray-700/50">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            rows={3}
                            placeholder="Any specific notes about these cards..."
                            maxLength={200}
                          />
