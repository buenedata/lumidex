-- ── Restore article content for all 6 seed stories ───────────────────────────
-- Run this if story content was accidentally overwritten with an empty array
-- (e.g. by the admin edit page before the content-fetch bug was fixed).

UPDATE stories SET content = '[
...
WHERE slug = 'top-10-value' AND (content = '[]'::jsonb OR content IS NULL);
...
WHERE slug = 'never-been-graded' AND (content = '[]'::jsonb OR content IS NULL);
...
WHERE slug = 'mega-evolution-whats-next' AND (content = '[]'::jsonb OR content IS NULL);
...
WHERE slug = 'ken-sugimori-spotlight' AND (content = '[]'::jsonb OR content IS NULL);
...
WHERE slug = 'price-movers-this-week' AND (content = '[]'::jsonb OR content IS NULL);
...
WHERE slug = 'regional-championship-results' AND (content = '[]'::jsonb OR content IS NULL);
